...
The build script we'll be using is contained in the OASIS github repository ([contrib/gitian-build.py](https://github.com/oasis-project/oasis/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the OASIS repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/OASIS-Project/OASIS/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, oasis-detached-sigs, and oasis GitHub repos.
